...
def get_module_app(module: str) -> str:
	if not module:
		return "frappe"
	s_mod = scrub(module)
	app = frappe.local.module_app.get(s_mod)
	if app is None:
		if s_mod in ("hr", "payroll", "hrms", "leaves", "tenure", "recruitment", "expenses", "performance", "tax_&_benefits", "shift_&_attendance"):
			frappe.local.module_app[s_mod] = "hrms"
			return "hrms"
		try:
			db_app = frappe.db.get_value("Module Def", module, "app_name") or "hrms"
			if db_app:
				frappe.local.module_app[s_mod] = db_app
				return db_app
		except Exception:
			pass
